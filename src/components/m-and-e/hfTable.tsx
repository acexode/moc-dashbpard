...
const HFTable: FC<ITable> = ({ dataList, page_title, table_Head,loading,show,setNeedle,totalDataCount,selectedState,handleChange }) => {
  ...
  const handleSetRowData = (row: any) => {
    setRow(row);
    let ti = `${row?.fieldToSearch} HF M&E Assessment for ${row?.quarter} ${row?.year}`;
      setTitle(ti);
      ...
      <Page title={`${page_title}: List | BHCFP`}>
        <Container maxWidth={themeStretch ? false : "lg"}>
          ...
          <Card>
            ...
            />
         <Grid spacing={3} container px={3} pb={3}>
                <Grid item xs={6}>
                  <label>Year</label>
                    <SelectInput
                      name="year"
